...
class CirceCodecBuilder[U <: Universe](val u: U) extends CodecBuilder {
  ...
  val anyEncoder = q"""
    def anyEncoder: Encoder[Any] = Encoder.instance((a: Any) => a match {
      ...
      case s: Array[Any]     @unchecked => s.asJson(Encoder.encodeIterable(anyEncoder, implicitly))
      case s: Seq[Any]       @unchecked => s.asJson(Encoder.encodeIterable(anyEncoder, implicitly))
      case ma: Map[String, Any] @unchecked => ma.asJson(Encoder.encodeMap(KeyEncoder.encodeKeyString, anyEncoder))
    })
  """
